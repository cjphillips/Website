...
BST.prototype.removeFromLinkedList = function(node)
{
...
BST.prototype.getLevel = function(val, node, level)
{
    if (val === undefined || val == null)
    {
        return -1;
    ...
    var leftLevel = this.getLevel(val, node.left, level + 1);
    var rightLevel = this.getLevel(val, node.right, level + 1);
    
    if (leftLevel < 0) return rightLevel;
    return leftLevel;
};
...
BST.prototype.inOrderForEach = function(callback, node)
{
...
BST.prototype.getUncle = function(node)
{
...
BST.prototype.getGrandparent = function(node)
{
...
BST.prototype.getSibling = function(node)
{
    if (node === null || node.parent === null) return null;
...
BST.prototype.getNode = function(val, node)
{
    if (node === undefined)
    {
        node = this.m_root;
    ...
    var res = this.getNode(val, node.left);
    if (res === null)
    {
        res = this.getNode(val, node.right);
    }
    return res;
};
...
{
    var cur = this.m_first, prev;
    while(cur)
    {
        cur.left = cur.right = cur.previous = null;
        prev = cur;
        cur = cur.next;
        prev.next = null;
        prev = null;
...
BST.prototype.countLevels = function(node)
{
    if (node === undefined)
    {
        node = this.m_root;
    ...
    var left = this.countLevels(node.left);
    var right = this.countLevels(node.right);
